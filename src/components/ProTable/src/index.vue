    ...
    <div class="table-header">
      <div class="header-button-lf">
        ...
        <render-btn
          ...
          :render="renderButtons({ selectedListIds, selectedList, isSelected })"
        />
        <!-- 插槽渲染 -->
        <slot
          v-else
...
import renderBtn from '../components/render-btn'
...
const renderButtons = (scope: any) => {
  return props.buttons.map((v) => {
    return (
      <el-button type={v.type ?? 'primary'} onClick={() => v.handleClick(scope)}>
        {v.label}
      </el-button>
    )
  })
